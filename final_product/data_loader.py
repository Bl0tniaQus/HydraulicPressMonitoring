...
def loadObservation(observation_dict):
    ...
    return values
def getFeaturesForObservation(dataframe, measurements, sensor_name):
    ...
    p2p = maxes - mins
    RMS = np.apply_along_axis(rms, 0, data)
    variance = np.apply_along_axis(moment, 0, data, 2)
    sd = np.sqrt(variance)
    moment3 = np.apply_along_axis(moment, 0, data, 3)
    moment4 = np.apply_along_axis(moment, 0, data, 4)
    moment5 = np.apply_along_axis(moment, 0, data, 5)
    moment6 = np.apply_along_axis(moment, 0, data, 6)
    moment7 = np.apply_along_axis(moment, 0, data, 7)
    moment8 = np.apply_along_axis(moment, 0, data, 8)
    moment9 = np.apply_along_axis(moment, 0, data, 9)
    moment10 = np.apply_along_axis(moment, 0, data, 10)
    krt = np.apply_along_axis(kurtosis,0,data)
    np.nan_to_num(krt)
    skw = np.apply_along_axis(skew, 0, data)
    np.nan_to_num(skw)
    variation = np.divide(sd, means)
    dataframe[sensor_name+"_mean"] = means
    dataframe[sensor_name+"_min"] = mins
    ...
    for freq in frequency_ranges[sensor_name]:
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_mean"] = np.apply_along_axis(mean_f, 0, data, fs, freq[0], freq[1])
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_max"] = np.apply_along_axis(max_f, 0, data, fs, freq[0], freq[1])
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_sd"] = np.apply_along_axis(sd_f, 0, data, fs, freq[0], freq[1])
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_variance"] = np.apply_along_axis(variance_f, 0, data, fs, freq[0], freq[1])
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_RMS"] = np.apply_along_axis(rms_f, 0, data, fs, freq[0], freq[1])
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_skewiness"] = np.apply_along_axis(skew_f, 0, data, fs, freq[0], freq[1])
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_kurtosis"] = np.apply_along_axis(kurtosis_f, 0, data, fs, freq[0], freq[1])
        dataframe[sensor_name+f"_f({freq[0]}-{freq[1]})_variation"] = np.apply_along_axis(variation_f, 0, data, fs, freq[0], freq[1])
    return dataframe
def rms(row):
